...
class MicroAverage(Score):
    ...
    largest in the annotated set, which `docmatch corpus` counts, so averaging
    each document's F1 would let a one-row table weigh as much as a hundred-row
    one and the number would track table size rather than extraction. Summing
    the counts gives every row, and every header value, the same weight.
    """
