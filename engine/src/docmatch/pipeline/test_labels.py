...
@pytest.fixture
def pinned_copies() -> labels.Labels:
    """The labels backend over the fixture loop run's manifest and copies."""
    return labels.load(
...
def test_the_reading_is_the_documents_labels(
    pinned_copies: labels.Labels, document_id: str
) -> None:
    """Found by the PDF alone: the document id the loop hands it is its own."""
    read = pinned_copies.extract(Document("17", COPIES / f"{document_id}.pdf", 1))
...
def test_a_reading_costs_nothing(pinned_copies: labels.Labels) -> None:
    read = pinned_copies.extract(Document("17", COPIES / "eval0005.pdf", 1))
