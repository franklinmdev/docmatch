"""The model behind the arms, an embedder, behind one interface.

An embedder maps texts to vectors; the arms take it through the interface,
so the tests drive them with a deterministic fake and the command loads the
real one in one place, here (#129). The reranker ADR 0001 measured was
dropped at `27bc70d` and deleted with its arm (#143).
...
was the better setting for the pair. It stays at ten with the reranker gone
so the arms' latency stays comparable with the row at `27bc70d`. Model load
is timed once and reported beside the table, never inside a query's latency.
...
def load_models() -> Loaded:
    """The real embedder from the local cache or the hub, at the pinned revision,
