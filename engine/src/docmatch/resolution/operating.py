...
The threshold is set by the procedure in `run`, on the development slice:
...
KEEP = 0.95
"""The share of answerable development queries the threshold keeps (#151)."""
...
class Resolved(BaseModel):
    """One line's resolution: its top-1 SKU at or above the threshold, and
...
def at_threshold(fetched: Fetched, threshold: float = OPERATING_THRESHOLD) -> Resolved:
    """The hybrid's top-1 answer kept at the threshold."""
    if not fetched.answers:
        return Resolved(sku=None, score=None)
    top = fetched.answers[0]
    return Resolved(sku=top.sku if top.score >= threshold else None, score=top.score)
...
def resolve_line(store: Store, embedder: Embedder, description: str) -> Resolved:
    ...
    return at_threshold(hybrid(store, embedder, description, DEPTH))
