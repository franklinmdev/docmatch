...
statement and the fusion. Model load, embedding the catalog and building the
HNSW index are each timed once and reported beside the table, never per
query. The model is loaded through the loader given, after the database has
answered with both extensions and only when there is something to resolve,
so a database that does not answer, or one missing an extension, is reported
