...
1. Its headline top-1 (over the scored answerable queries, exact weight
   ...
   the hybrid arm's at the d the same sweep chose, both over the same scored
   queries.
...
threshold. Weighing separability lexicographically inside the top-1 band, or
as a veto, was rejected: the first turns a diagnostic into a criterion, the
second vetoes on a figure Phase 4 may not use.
...
(10, 25, 50 pairs, about 47, 102 and 208 ms mean, p95 estimated at about 1.6
times the mean from the embedder's ratio, so about 340 ms at N = 50) fits under
it, so the ceiling only bites on a surprise, which is what a pre-registered
ceiling is for. The estimate is a probe-based guess; the rule uses the measured
p95 and nothing else. A
...
resolves real lines with. Dropped means the rerank arm and its N sweep are
...
permanent ablation row was rejected as about eight minutes a run and the
cross-encoder in CI with nothing using it; keeping it behind a flag off by
default was rejected as a path nothing measures.

**Where it lives.** The two thresholds are constants in code beside d and N.
`docmatch resolve` prints the verdict beside the headline, with both
measurements against both constants, the way the pairing floor prints its
constant beside the value the procedure measures (rule 4: code decides). The
README's Phase 3 section states the rule, links this file and the commit whose
run produced the verdict. Recording the rule on the ticket alone, or as README
prose alone, was rejected: a repo reader would have to leave for GitHub to see
the rule preceded the number, and prose computes nothing, so the verdict would
not reproduce with the row.
